...
export default ({ onEdit, onDelete, CurrencyMap }) => {
  ...
  const readPartyType = (row) => (
    <Badge color={BooleanColors[row.PartyType]}>
      {PartyTypeOptions.find((x) => x.value == row.PartyType).label}
    </Badge>
  );
  const readDisabled = (row) => (
    <Badge color={BooleanColors[row.Disabled]}>
      {stringToCapitalize(`${row.Disabled}`)}
    </Badge>
  );
  const readIsDefault = (row) => (
    <Badge color={BooleanColors[row.IsDefault]}>
      {stringToCapitalize(`${row.IsDefault}`)}
    </Badge>
  );
  ...
  const minWidths = Array.from({ length: 8 }, () => "25%");
  const cells = [
    null,
    readPartyType,
    null,
