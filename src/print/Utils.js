...
export const buildHTMLTable = ({
  ...
  for (let i = 0; i < data?.length; i++) {
    Table += "<tr>";
    if (numberedRow) Table += `<td>${shift + i + 1}</td>`;
    for (let j = 0; j < _keys.length; j++) {
...
export const buildHTMLTableV2 = ({ data = [], numberedRow = false }) => {
  ...
  for (let i = 0; i < data?.length; i++) {
    Table += "<tr>";
    if (numberedRow) Table += `<td>${i + 1}</td>`;
    for (let j = 0; j < _keys.length; j++) {
      Table += `<td>${fixValue(data[i][_keys[j]])}</td>`;
    }
    Table += "</tr>";
...
export const dividedData = ({
  ...
  content += buildHTMLTable({
    data: data.slice(0, First_WINDOW_LENGTH),
    numberedRow,
  });
...
const getMentionValue = ({ mention, PrintKeys, data }) => {
  try {
    ...
    return data.hasOwnProperty(PrintKeys[mention])
      ? data[PrintKeys[mention]]
      : PrintKeys[mention];
  } catch (error) {
...
export const replaceMentions = ({ template, data, PrintKeys }) => {
  try {
    ...
    for (let index = 0; index < template.length; ) {
      const [mention, end] = getSubstring({
        string: template.slice(index),
        start: '<a data-mention="true">',
        end: "</a>",
        inner: true,
      });
      tempString = tempString.replace(
        mention,
        getMentionValue({ mention, PrintKeys, data })
      );
      index += end;
    }
    return tempString;
...
const getAttributes = (string) => {
  ...
  for (let index = 0; index < string.length; ) {
    const [attribute, end] = getSubstring({
...
export const replaceTables = ({ template, data, PrintKeys }) => {
  try {
        ...
        return {
          [`${x}`]: data[`${x}`].length
            ? buildHTMLTableV2({
                numberedRow: true,
                data: data[`${x}`].map((y) =>
                  getAttributeFromObject(y, attributesTableMap[x])
                ),
              })
            : "",
        };
    ...
    Object.keys(htmlTablesMap).forEach((x) => {
      tempString = replaceTable({
        name: x,
        realTable: htmlTablesMap[x],
        template,
        tempString,
      });
    });
