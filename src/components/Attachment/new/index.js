...
import {
  ...
  clearAttachments,
  setAttachments,
  replaceSignleAttachment,
  renameSignleAttachment,
  replaceMultipleAttachments,
  renameMultipleAttachments,
} from '../store/action';
...
const index = ({ isModalOpen, handleToggleModel }) => {
  let Attachment = useSelector((state) => state.Attachment);
  const dispatch = useDispatch();

  let attachmentNames = useMemo(
    () => Attachment?.map((x) => x?.name),
    [Attachment]
  );
      ...
      if (acceptedFiles && acceptedFiles?.length > 0) {
        if (acceptedFiles.length === 1) {
          if (attachmentNames.includes(acceptedFiles[0]?.name)) {
            Swal.fire(
              ...
              if (result.isConfirmed) {
                // Replace Single File
                dispatch(replaceSignleAttachment(acceptedFiles[0]));
            ...
            dispatch(addAttachments(acceptedFiles[0]));
          }
        } else {
          // upload mutltiple
          ...
          if (duplicateNames?.length > 0) {
            ...
            dispatch(setAttachments([...Attachment, ...acceptedFiles]));
          }
        }
      }
    },
  ...
  const onDelete = (row) => {
    dispatch(deleteAttachments(row?.Series));
  };
    ...
    >
      ...
      <ModalBody>
        <ModalContainer {...getRootProps()} className="uploadContainer">
          ...
          <CustomTable offlineData={Attachment} columns={_columns} />
        </ModalContainer>
        <h6 style={{ color: 'gray' }}>
