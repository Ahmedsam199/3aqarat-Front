...
import { Currency as Schema } from "@validation";
...
const POST = ({ onToggle, row, toggleFunc }) => {
  ...
  const onSubmit = async (values) => {
    ...
    dispatch(
      values.Series
        ? updateItem("Currency", values)
        : insertItem("Currency", values)
    )
      .then((res) => {
        toast.success("");
        clear();
      ...
      >
        <FormProvider {...methods}>
          <Form onSubmit={handleSubmit(onSubmit)}>
            {/* s</ModalHeader> */}
            <CustomFormInput name="CurrencyName" />
            <CustomFormInput name="Symbol" />
            <CustomFormSelect options={FormatOptions} name="Format" />
            <div className="mt-1">
              <Button
                color="primary"
                type="submit"
