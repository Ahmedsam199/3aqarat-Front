...
import { Setup_Permissions as Schema } from "@validation";
...
const POST = ({ onToggle, row, toggleFunc }) => {
  ...
  const onSubmit = async (values) => {
    ...
    dispatch(
      values.Series
        ? updateItem("Setup_Permission", values)
        : insertItem("Setup_Permission", values)
    )
      .then((res) => {
        toasty({ type: "success" });
        clear();
      ...
      >
        <FormProvider {...methods}>
          <Form onSubmit={handleSubmit(onSubmit)}>
            {/* s</ModalHeader> */}
            <CustomFormInput name="TerrorityName" />
