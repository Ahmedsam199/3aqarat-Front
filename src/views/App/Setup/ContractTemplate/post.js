...
const Post = () => {
  ...
  const PrintKeys = useMemo(
    () =>
      AllPrintKeys.filter((x) => x.Doctype == params?.doctype)
        .map((x) => x.PrintKeys)
        ?.reduce((s, x) => [...s, ...x]),
  ...
  const [
    ...
    dispatch,
  ] = useReducer(reducer, initialState(params));
  ...
  useEffect(() => {
    if (params.series) {
      dispatch({
        type: "initValues",
        payload: ContractTemplate.find((x) => x.Series === params.series),
      });
    } else {
      dispatch({
        type: "setHeader",
        payload: localStorage.getItem("imgBase64"),
      });
    }
  }, []);
  ...
  const applyTable = ({ name, attributes, old }) => {
    ...
    if (old) {
      ...
      dispatch({
        type: "setHtmlData",
        payload: editorRef.current.getContent() + table,
      });
              ...
              <FormGroup>
                ...
                <div className="color-container">
                  ...
                  >
                    <div
                      className="color"
                      style={{ background: tableHeadersColor }}
                    />
                  </div>
                  {displayColorPicker ? (
