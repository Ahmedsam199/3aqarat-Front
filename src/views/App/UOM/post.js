...
import { UOM as Schema } from '@validation';
...
const POST = ({ onToggle, row, toggleFunc }) => {
  ...
  const onSubmit = async (values) => {
    setLoading(true);
    dispatch(values.Series ? updateItem("UOM", values) : insertItem("UOM", values)).
      then(res => {
        toasty({ type: 'success' })
        clear()
        toggle()
      ...
      >
        <FormProvider {...methods}>
          <Form onSubmit={handleSubmit(onSubmit)}>
            {/* s</ModalHeader> */}
            <CustomFormInput name="UOM" />
            <CustomFormNumberInput name="ConverstionFactor" />
            <Button color="primary" type="submit" className="mr-1" disabled={loading || (row && !_write)}>
